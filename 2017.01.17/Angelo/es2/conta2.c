/**
 * While testing, note that copied files from es1 will not work.
 * You'll need to make new hard links.
 */
...
int main(int argc, char** argv)
{
    ...
    for (int i=0; i <= maxref; i++)
    {
        ...
        for (int j = 0; j < inodes_found_n; j++){
            if (files[j].ref == i)
                printf("%s, inode %d | ", files[j].name, files[j].inode);
        }
            ...
            if (dp->d_type == DT_REG) { /* If the entry is a regular file */
                ...
                for (int j = 0; j < inodes_found_n; j++) {
                    if (inodes_list[j] == fbuf.st_ino) {
                        files[j].ref++;
                        if (files[j].ref > maxref) maxref = files[j].ref;
                        found_inode = 1; 
                        break;
                    } 
                }
                ...
                if (found_inode == 0) {
                    //inode is new!!
                    inodes_found_n++;
                    files[inodes_found_n-1].name = malloc(64*sizeof(char));
                    strcpy(files[inodes_found_n-1].name, dp->d_name);
                    files[inodes_found_n-1].ref = 1;
                    files[inodes_found_n-1].inode = fbuf.st_ino;

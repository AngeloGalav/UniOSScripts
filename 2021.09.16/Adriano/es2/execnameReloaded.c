        ...
        if ( event->len ) {
            if ( event->mask & IN_CREATE ) {
                ...
                // strcpy(command, event->name);
                  ...
                  const char relativePath[1000]; // relative path of the file to delete
                  ...
                  while (pch != NULL) // copy the rest of the command to execute
                    ...
                    strcpy(relativePath, pathToFile); // complete(relative) path to the file we have to delete
                    strcat(relativePath, event->name);
